...
def get_items(start, page_length, price_list, item_group, pos_profile, search_term="",
				vehicle_make="", vehicles_model="", model_year="", engine=""):
	...
	if vehicle_make:
		vehicle_make_cond = f"""AND item.item_code in (
				SELECT parent FROM `tabCompatible with` WHERE vehicle_manufacturers = '{vehicle_make}')
			"""
	if vehicle_make and vehicles_model:
		vehicle_make_cond = f"""AND item.item_code in (
				SELECT parent FROM `tabCompatible with` 
				WHERE vehicle_manufacturers = '{vehicle_make}'
				AND vehicles_model = '{vehicles_model}'
				)
			"""
	if vehicle_make and vehicles_model and model_year:
		vehicle_make_cond = f"""AND item.item_code in (
			SELECT parent FROM `tabCompatible with` 
			WHERE vehicle_manufacturers = '{vehicle_make}'
			AND vehicles_model = '{vehicles_model}'
			AND year_model_v like '{model_year}'
			)
		"""
	if vehicle_make and vehicles_model and engine:
		vehicle_make_cond = f"""AND item.item_code in (
				SELECT parent FROM `tabCompatible with` 
				WHERE vehicle_manufacturers = '{vehicle_make}'
				AND vehicles_model = '{vehicles_model}'
	...
	items_data = frappe.db.sql("""
		...
		.format(
			...
			rgt=rgt,
			condition=condition,
			vehicle_make_cond=vehicle_make_cond,
